...
public class FilmController {
	...
	public ResponseEntity<List<FilmDTO>> getFilmovi(){
		...
		for(Film film:filmovi) {
			FilmDTO filmDTO=new FilmDTO(film.getId(),film.getNaziv(),film.getOpis(),film.getZanr(),film.getTrajanje(),film.getSrednja_ocjena());
	...
	@GetMapping(
			value="/sortOcjena",
			produces=MediaType.APPLICATION_JSON_VALUE)
	public ResponseEntity<List<FilmDTO>> sortOcjena(){
		List<Film> filmovi=this.filmService.orderOcjena();
	...
	public ResponseEntity<List<FilmDTO>> pretraga(@RequestBody FilmDTOPretraga f)throws Exception{
			...
			for (Terminski_raspored tr : projekcije) {
				...
				}if(f.getSrednjaOcjena()!=null) {
					if(Double.compare(f.getSrednjaOcjena(), tr.getFilm().getSrednja_ocjena())==0) {
						znak=1;
					}else {
						r=false;
				...
				if(f.getCijena()!=0) {
					...
					if(tr.getDatum_odrzavanja()!=null) {
						System.out.println(tr.getDatum_odrzavanja());
						System.out.println(f.getDatumOdrzavanja());
				...
				if(znak==1) {
					if(r==true) {
						FilmDTO filmDTO=new FilmDTO(tr.getFilm().getId(),tr.getFilm().getNaziv(),tr.getFilm().getOpis(),tr.getFilm().getZanr(),tr.getFilm().getTrajanje(),tr.getFilm().getSrednja_ocjena());
			...
			//ako nema terminski listi idem po filmovima ali tada zanemarujem polja datum i cijenu jer te podatke ni nemam o tim filmovima
			for (Film f1 : filmovi) {
				r=true;
				if(f.getNaziv()!="") {
					...
					if(Double.compare(f.getSrednjaOcjena(), f1.getSrednja_ocjena())==0) {
						znak=1;
					}else {
						r=false;
				...
				if(znak==1) {
					if(r==true) {
						FilmDTO filmDTO=new FilmDTO(f1.getId(),f1.getNaziv(),f1.getOpis(),f1.getZanr(),f1.getTrajanje(),f1.getSrednja_ocjena());
	...
	public ResponseEntity<List<Terminski_rasporedDTO>> prikaz(@PathVariable(name="id") Long id){
		...
		for (Terminski_raspored t : projekcije) {
			...
			tr.setSrednjaOcjena(t.getFilm().getSrednja_ocjena());
