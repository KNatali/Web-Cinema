...
public class BioskopController {
	...
	public ResponseEntity<BioskopDTOAdd> dodaj(@RequestBody BioskopDTOAdd b) throws Exception	{
		...
		//ako je unijet nepostojeci menadzer
		if(menadzer==null) {
	...
	@PostMapping(
		...
		public ResponseEntity<SalaDTO> dodaj(@RequestBody SalaDTO s) throws Exception	{
			...
			//ako je unijeti bioskop nepostojeci
			if(b==null) {
				return new ResponseEntity<>(HttpStatus.NOT_FOUND);
			}
			else {
	...
	public ResponseEntity<Raspored_filmova> dodajRaspored(@RequestBody Terminski_rasporedDTO t)throws Exception{
			...
			Film f1=new Film(f.getNaziv(), f.getOpis(), f.getZanr(), f.getTrajanje(), f.getSrednja_ocjena());
			f1.setId(f.getId());
			this.filmService.save(f1);
			te.setFilm(f1);
			te.setBroj_rezervacija(0);
			te.setCijena(t.getCijena());
			te.setDatum_odrzavanja(t.getDatumOdrzavanja());
			te.setVrijeme_pocetka(t.getVrijemePocetka());
	...
	public ResponseEntity<Terminski_rasporedDTO> repertoar(@PathVariable(name="id") Long id){
			...
			t1.setVrijemePocetka(t.getVrijeme_pocetka());
			t1.setCijena(t.getCijena());
	...
	@PostMapping(
			value="/repertoar/izmjenjivanje",
			consumes=MediaType.APPLICATION_JSON_VALUE,
			produces=MediaType.APPLICATION_JSON_VALUE)
	public ResponseEntity<Terminski_rasporedDTO> repertoarIzmjena(@RequestBody Terminski_rasporedDTO t)throws Exception{
			...
			tr.setVrijeme_pocetka(t.getVrijemePocetka());
